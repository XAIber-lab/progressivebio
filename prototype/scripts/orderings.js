 ...
 */
function sortNodes(orderKey, nodes, edges) {
    sortAlphabetically(nodes);
    //console.log("nodes before: " + nodes.map(n => n.id))
    switch (orderKey) {
        case "adjacency":
    ...
    //console.log("nodes after " + orderKey + " sorting: " + nodes.map(n => n.id))
}
...
function sortEdges(orderKey, nodes, edges) {
    ...
    //console.log("edges before: " + edges.map(n => n.id))
    switch (orderKey) {
        case "alphabetical":
            sortAlphabetically(edges);
            break;
    ...
    //console.log("edges after " + orderKey + " sorting: " + edges.map(n => n.id))
}
...
function sortByRCM(nodes, edges) {
    /* // problem with this rcm implementation is that its inherently random, so we cannot reproduce orderings
    ...
    */
    ...
    nodes.forEach(function (n) {
        degrees[n.id] = edges.filter(e => e.source === n.id || e.target === n.id).length;
    ...
    while (notVisited.length) {
        ...
        while (queue.length) {
            let toSort = [];
            let neighbors = edges.filter(e => e.source === queue[0] || e.target === queue[0])
                .map(function (e) {
                    if (e.target === queue[0]) {
                        return e.source;
                    } else {
...
function sortBySimulatedAnnealing(nodes, edges) {
    ...
    let rng = new LCG(103);
    while (temperature > 1) {
        let neighborLayout = generateNeighbor(currentLayout, rng);
        let neighborEnergy = calculateEnergy(neighborLayout, edges);
        if (acceptNeighbor(currentEnergy, neighborEnergy, temperature, rng)) {
...
function generateNeighbor(layout, rng) {
    let newLayout = [...layout];
    const index1 = Math.floor(rng.random() * newLayout.length);
    let index2 = Math.floor(rng.random() * newLayout.length);
    while (index1 === index2) { // Ensure we have two distinct indices
        index2 = Math.floor(rng.random() * newLayout.length);
    }
    // Swap the positions of two nodes
    [newLayout[index1], newLayout[index2]] = [newLayout[index2], newLayout[index1]];
    return newLayout;
}
function acceptNeighbor(currentEnergy, neighborEnergy, temperature, rng) {
